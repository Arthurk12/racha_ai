...
export default function ExpenseHistory({ users, expenses, removeExpense, updateExpense, currentUserId, isAdmin, onShowToast, pendingId }: ExpenseHistoryProps) {
  ...
  const saveEditing = (id: string) => {
    ...
    if (isNaN(amount) || editDate.length !== 10 || !editDescription.trim()) {
        onShowToast('Dados inválidos', 'error')
        return
    }

    if (editParticipants.length === 0) {
        onShowToast('Selecione pelo menos um participante', 'error')
        return
    }

    const [day, month, year] = editDate.split('/')
    if (!day || !month || !year) {
        onShowToast('Data inválida', 'error')
        return
    }
    ...
    <div className="bg-slate-800 p-6 rounded-lg shadow-xl border border-slate-700 mt-6 md:mt-0">
      ...
      >
        ...
        <div className="flex items-center gap-3">
            <span className="text-sm bg-slate-700 px-2 py-1 rounded border border-slate-600 text-slate-300">
                {expenses.length}
            </span>
      ...
      {isExpanded && (
        <>
        <ul className={`space-y-4 mt-4 overflow-y-auto pr-2 custom-scrollbar transition-all duration-300 ${isMaximized ? 'max-h-[75vh]' : 'max-h-96'}`}>
            ...
            return (
                ...
                className={`flex justify-between items-start p-3 rounded-lg border transition-all hover:bg-slate-700/50 ${
                    ...
                    ? 'bg-green-900/20 border-green-500/50 border-2 border-dotted flex-row-reverse'
                    : isMyExpense 
                ...
                <div className={`flex items-start gap-3 flex-1 min-w-0 ${expense.isSettlement ? 'flex-row-reverse text-right' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className={`text-xs text-slate-500 ${expense.isSettlement ? '' : 'mt-1 min-h-[24px]'}`}>
                           {isEditing ? (
                              !expense.isSettlement && (
